...
def get_proxied_page(target_url, max_retries_per_provider=3, session_id=None):
    ...
    while True:
        ...
        # If a single provider fails too many times, swap it out
        if network_retries >= max_retries_per_provider:
            print_time(f"  ❌ {provider.upper()} failed {max_retries_per_provider} times. Forcing failover...")
            CURRENT_KEY_INDEX += 1
            network_retries = 0
            continue
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Check for obvious CAPTCHAs
                if "px-captcha" in page_text or "Press & Hold" in page_text or "Robot or human" in page_text:
                ...
                print_time(f"  ❌ {provider.upper()} rejected the payload (400). Switching providers...")
                CURRENT_KEY_INDEX += 1
                network_retries = 0
                continue
